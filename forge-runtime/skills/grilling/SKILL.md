...
呼叫方選擇 `converge` 時，只檢查進入 Deep Retrieval 所需的真正客觀知識或證據盲點；可合理預設的實作細節不算阻塞，最多提出一題。沒有盲點時輸出 `READY_FOR_DEEP`，且 `questions` 必須是空陣列；若提出一題，人類回答後系統直接進入 `DEEP_KNOWLEDGE_RETRIEVAL`，不得再開新的 Grill round、checkpoint 或第二題。
